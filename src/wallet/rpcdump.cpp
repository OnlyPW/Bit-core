...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address_or_nickname\"   (string, required) The BIT address or nickname for the private key\n"
            "\nResult:\n"
            "\"key\"                (string) The private key\n"
            "\nExamples:\n"
            + HelpExampleCli("dumpprivkey", "\"myaddress\"")
            + HelpExampleCli("dumpprivkey", "\"bit_dev\"")
            + HelpExampleCli("importprivkey", "\"mykey\"")
    ...
    if (!address.SetString(strAddress)) {
        ...
        if (!nicknameDB || !nicknameDB->ReadNickname(normalized, info)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid BIT address or nickname");
        }
