...
static const unsigned char BIT_NICKNAME_MEMO_MAGIC[] = {'B', 'M', 'E', 'M', '1'};
static const unsigned char BIT_NICKNAME_MEMO_VERSION = 1;
static const size_t BIT_NICKNAME_MEMO_MAX_DATA_LEN = 48;
...
void ValidateNicknameMemoDataOrThrow(const NicknameMemoType type, const std::string& data)
{
    ...
    if (data.size() > BIT_NICKNAME_MEMO_MAX_DATA_LEN) {
        throw JSONRPCError(RPC_INVALID_PARAMETER,
                           strprintf("Memo is too long (max %d bytes)", BIT_NICKNAME_MEMO_MAX_DATA_LEN));
    }
...
std::vector<unsigned char> BuildNicknameMemoPayload(const std::string& data, const NicknameMemoType type)
{
    ...
    payload.insert(payload.end(), BIT_NICKNAME_MEMO_MAGIC,
                   BIT_NICKNAME_MEMO_MAGIC + sizeof(BIT_NICKNAME_MEMO_MAGIC));
    payload.push_back(BIT_NICKNAME_MEMO_VERSION);
...
{
    const size_t headerSize = sizeof(BIT_NICKNAME_MEMO_MAGIC) + 1 + 1 + 1;
    ...
    if (!std::equal(BIT_NICKNAME_MEMO_MAGIC,
                    BIT_NICKNAME_MEMO_MAGIC + sizeof(BIT_NICKNAME_MEMO_MAGIC),
                    payload.begin())) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Memo payload magic mismatch");
    }

    const unsigned char version = payload[sizeof(BIT_NICKNAME_MEMO_MAGIC)];
    if (version != BIT_NICKNAME_MEMO_VERSION) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Unsupported memo payload version");
    }

    const unsigned char rawType = payload[sizeof(BIT_NICKNAME_MEMO_MAGIC) + 1];
    const unsigned char dataLen = payload[sizeof(BIT_NICKNAME_MEMO_MAGIC) + 2];
...
static CBitcoinAddress ResolveActiveNicknameAddress(const std::string& nicknameInput, std::string* normalizedNicknameOut = nullptr)
{
    ...
    if (!Nicknames::IsValidNormalizedNickname(normalizedNickname)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid BIT address or nickname: ") + nicknameInput);
    }
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw runtime_error(
            ...
            "                             The recipient will receive less BIT than you enter in the amount field.\n"
            "\nResult:\n"
...
UniValue sendtonickname(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 7)
        throw runtime_error(
            ...
            "\"txid\"                  (string) The transaction id.\n"
            "\nExamples:\n"
            + HelpExampleCli("sendtonickname", "\"bit_dev\" 0.1")
            + HelpExampleCli("sendtonickname", "\"bit_dev\" 0.1 \"donation\" \"seans outpost\"")
            + HelpExampleCli("sendtonickname", "\"bit_dev\" 0.1 \"\" \"\" true \"12345678\" \"numeric\"")
            + HelpExampleRpc("sendtonickname", "\"bit_dev\", 0.1, \"donation\", \"seans outpost\"")
        );
...
UniValue decodenicknamememo(const JSONRPCRequest& request)
{
    ...
    result.pushKV("version", static_cast<int>(BIT_NICKNAME_MEMO_VERSION));
    return result;
}
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw runtime_error(
            ...
            "    {\n"
            "      \"destination\":amount   (numeric or string) The BIT address or active nickname is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            "                           Those recipients will receive less BIT than you enter in their corresponding amount field.\n"
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    ...
    if (pwalletMain->IsCrypted() && (request.fHelp || request.params.size() != 2))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending BIT\n"
            "\nArguments:\n"
...
UniValue lockunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending BIT.\n"
...
UniValue bumpfee(const JSONRPCRequest& request)
{
    ...
    if (totalFee > 0) {
        ...
        // otherwise bump the fee by 1 BIT.
        else {
