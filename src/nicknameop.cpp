...
static const char BIT_NICKNAME_MAGIC[] = {'B', 'I', 'T', '1'};
...
bool BuildNicknameOpScript(const NicknameOperation& operation, CScript& scriptOut, std::string* reason)
{
    ...
    payload.write(BIT_NICKNAME_MAGIC, sizeof(BIT_NICKNAME_MAGIC));
...
bool ExtractNicknameOperation(const CScript& script, NicknameOperation& operationOut, std::string* reason)
{
    ...
    char magic[sizeof(BIT_NICKNAME_MAGIC)];
    try {
    ...
    if (!std::equal(magic, magic + sizeof(magic), BIT_NICKNAME_MAGIC)) {
        if (reason) {
