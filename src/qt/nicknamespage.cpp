// Copyright (c) 2026 The BIT developers
...
QString FormatBitAmount(const QString& value)
{
    ...
    if (fracPart.isEmpty()) {
        return sign + intPart + " BIT";
    }

    return sign + intPart + "." + fracPart + " BIT";
}
...
QString RpcAmountToQString(const UniValue& value)
{
    ...
    if (value.isNum()) {
        return FormatBitAmount(QString::fromStdString(value.getValStr()));
    }
    return FormatBitAmount(RpcValueToQString(value));
}
...
QString FormatBitAmountFromNative(const CAmount amount)
{
    return FormatBitAmount(QString::fromStdString(FormatMoney(amount)));
}
...
QString BondInfoForStatus(const NicknameInfo& info, const Nicknames::Status status)
{
    ...
    if (status == Nicknames::Status::BOND_CLAIMABLE) {
        return QObject::tr("claimable %1").arg(FormatBitAmountFromNative(info.bondAmount));
    }
    return QObject::tr("locked %1").arg(FormatBitAmountFromNative(info.bondAmount));
}
} // namespace
      ...
      currentLookupClaimable(false)
{
    ...
    nicknameEdit->setPlaceholderText(tr("e.g. bit_dev or @bit_dev"));
...
void NicknamesPage::onLookupClicked()
{
    ...
    currentBondAmount = FormatBitAmountFromNative(info.bondAmount);
    pricingValue->setText(tr("Price is shown only in Buy dialog."));
...
void NicknamesPage::showNicknameDetailsDialog(const QString& nickname)
{
    ...
    dialog.setStyleSheet(
        ...
        "QTextEdit { background: #ffffff; border: 1px solid #d0d0d0; border-radius: 8px; color: #000000; }"
...
bool NicknamesPage::confirmAction(const QString& title, const QString& text) const
{
    ...
    box.setStyleSheet(
        ...
        "QLabel { color: #000000; min-width: 360px; }"
        "QPushButton { min-width: 92px; min-height: 30px; }");
                                    ...
                                    QString& payoutOut)
{
    ...
    dialog.setStyleSheet(
        "QDialog { background: #ffffff; }"
        "QLabel { color: #000000; }"
        "QLabel#meta { color: #000000; }"
        "QFrame#headerBox { background: #ffffff; border: 1px solid #d0d0d0; border-radius: 10px; }"
        "QLabel#headerTitle { color: #000000; font-size: 14px; font-weight: 700; }"
        "QLabel#headerSub { color: #000000; }"
        "QFrame#feeBox { background: #ffffff; border: 1px solid #d0d0d0; border-radius: 10px; }"
        "QLabel#feeLabel { color: #000000; }"
        "QLabel#feeValue { color: #000000; font-weight: 700; }"
        "QLabel#countdownLabel { color: #000000; font-weight: 600; }"
        "QProgressBar#countdownProgress { border: 1px solid #d0d0d0; border-radius: 4px; background: #ffffff; }"
        "QProgressBar#countdownProgress::chunk { background: #ff8c00; border-radius: 3px; }"
                                             ...
                                             QString& newPayoutOut)
{
    ...
    dialog.setStyleSheet(
        ...
        "QLabel#headerTitle { color: #000000; font-size: 14px; font-weight: 700; }"
        "QLineEdit { min-width: 520px; }"
        "QLineEdit[readOnly=\"true\"] { background: #f7f7f7; color: #000000; border: 1px solid #d0d0d0; }"
        "QPushButton { min-width: 110px; min-height: 32px; }");
                                         ...
                                         QString& pubKeyOut)
{
    ...
    QLineEdit* input = new QLineEdit(&dialog);
    input->setPlaceholderText(tr("BIT address or compressed pubkey hex"));
    layout->addWidget(input);
...
void NicknamesPage::onRenewClicked()
{
    ...
    if (!promptDelayedConfirmationDialog(
            ...
            tr("Nickname: %1\nRenewal fee: %2\nAdditional locked bond: %3\nBond after renewal: %4")
                ...
                .arg(FormatBitAmountFromNative(projectedBondAfterRenew)),
            2)) {
        return;
    }
