...
- **0.01 BIT per kilobyte** transaction fee
- **0.01 BIT** dust limit (discard threshold)
- **0.001 BIT** replace-by-fee increments
...
transaction only has to pay `0.01 / 1000 * 192 = 0.00192` BIT fee.

## Miner default inclusion policies

The default values for miners to include a transaction in a block has been set
to exactly the recommended fee of **0.01 BIT/kB.** Dust limits are defined by
...
The default minimum transaction fee for relay is set at **0.001 BIT/kB**,
...
- The hard dust limit is set at **0.001 BIT** - outputs under this value are
  invalid and rejected.
- The soft dust limit is set at **0.01 BIT** - sending a transaction with outputs
  under this value, are required to add 0.01 BIT for each such output, or else
